...
/**
 * myCustomSetenv - change or adds an environment variable
...
int myCustomSetenv(const char *name, const char *value, int overwrite)
{
	...
	if (new_var == NULL)
		return (-1);
	const_strcpy(new_var, name), const_strcat(new_var, "=");
	...
	while (old_environ[index] != NULL)
	{
		...
		{
			old_environ[index] = new_var, environ = old_environ;
			return (0);
		}
		else/*got rid of memory leak errors*/
		{
			free(new_var), free(old_environ);
			return (0);
		}
		index++; }
	...
	if (new_environ == NULL)
	{
		...
		return (-1); }
	old_environ = new_environ, free(old_environ);
	new_environ[index] = new_var, new_environ[index + 1] = NULL;
	environ = new_environ;
	return (0);
}
